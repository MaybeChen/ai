...
export async function POST(req: Request) {
  ...
  const result = streamText({
    ...
    prompt: [
      ...
      '可用 block.type 只有：headline、status-strip、paragraph、bullet-list、key-value-list、metric、flowchart、callout、thinking-steps。',
      'thinking-steps 用于展示给用户看的简短“思考过程/分析过程摘要”，只能写高层次、简洁、可展示的步骤，不要输出冗长隐私推理。',
      ...
      '建议尽量先输出 headline 或 thinking-steps，再补充后续 block。',
      '',
      '以下是用户输入的完整告警信息：',
      alarmInput,
    ].join('\n'),
